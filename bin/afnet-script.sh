...
cat << HELP
NAME
    bin/afnet-script.sh - main script

SYNOPSIS
    bin/afnet-script.sh [....]
    bin/afnet-script.sh --new
    bin/afnet-script.sh --killarg [...]
    bin/afnet-script.sh --killsudo [...]
    bin/afnet-script.sh --kill
    bin/afnet-script.sh --rsync
    bin/afnet-script.sh --rsyncp

DESCRIPTION
    This mounts an Amiga FTP server on unix filesystem
    and does some sortof backups if you want via rsync

    It generally prompts user for args/values so we
    run it with expect instead automatically.
...
CHANGELOG
    ...
    Wed Mar  2 23:07:05 GMT 2022
    Getting it ready for packaging, remove local env
HELP
}
# ----------------------------------------------------- #
# Settings
...
function mycurl {
    ...
    if [[ "$aAMIGA_HOST" == "localhost" ]]; then
      # assume a unix test env without all the : that amigas use
      export aAMIGA_DIR=`echo $aAMIGA_DIR | sed 's#:##g'`
    fi
    ...
    then
        ...
        echo "ERROR ${aAMIGA_HOST} didn't reply" >> $AFHOME/etc/$AF_TEXT_ERRORS
        cat $AFHOME/etc/$AF_TEXT_ERRORS | sort | uniq  >> $AFHOME/etc/${AF_TEXT_ERRORS}.1
        mv $AFHOME/etc/${AF_TEXT_ERRORS}.1 $AFHOME/etc/${AF_TEXT_ERRORS}
    fi
    echo -e "\n${HIGHLIGHT}Starting CURLFTPS for ${aUNIX_TOPDIR}${NOHIGHLIGHT}"
    rm -f nohup.out 
    echo "curlftpfs -o nonempty -o user="${aAMIGA_LOGIN}" -s ftp://${aAMIGA_HOST}/${aAMIGA_DIR} ${aUNIX_TOPDIR}"
    mkdir -p ${aUNIX_TOPDIR}
    #nohup curlftpfs -o nonempty -s ftp://${aAMIGA_LOGIN}@${aAMIGA_HOST}/${aAMIGA_DIR} ${aUNIX_TOPDIR}
    curlftpfs -o nonempty -o user="${aAMIGA_LOGIN}" -s ftp://${aAMIGA_HOST}/${aAMIGA_DIR} ${aUNIX_TOPDIR}
    SUCCESS=$?
    if [ $SUCCESS -eq 0 ]
    then
        mylistcurl
      ...
      echo -e "\n>>>>>>>>>>>>>>>>>>>>>>\nThe following directories are mounted:\n`mount | grep curl | awk '{print $3}'`"
        ...
        curlcommand="curlftpfs -o nonempty -o user="${aAMIGA_LOGIN}" -s ftp://${aAMIGA_HOST}/${aAMIGA_DIR} ${aUNIX_TOPDIR}"
        curlftpfs -o nonempty -o user="${aAMIGA_LOGIN}" -s ftp://${aAMIGA_HOST}/${aAMIGA_DIR} ${aUNIX_TOPDIR} > nohup.out 2>&1
        sleep 2
        echo "ERROR: curl command failed [$curlcommand] with error [`cat nohup.out | tr -d '\n'`]. "
        echo "ERROR: curl command failed [$curlcommand] with error [`cat nohup.out | tr -d '\n'`]. " >> $AFHOME/etc/$AF_TEXT_ERRORS
        cat $AFHOME/etc/$AF_TEXT_ERRORS | sort | uniq  >> $AFHOME/etc/${AF_TEXT_ERRORS}.1
        mv $AFHOME/etc/${AF_TEXT_ERRORS}.1 $AFHOME/etc/${AF_TEXT_ERRORS}
    fi
}
